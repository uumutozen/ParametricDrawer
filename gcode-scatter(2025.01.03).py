...
def interpolate_angles(e_initial, e_terminal, steps=100):
    e_interp = np.linspace(e_initial, e_terminal, steps)
    return e_interp
...
# Initial Values
# [G*,X,Y,Z,E,F,I,J,P,Q]
gcode_list = [["G0", 0.0, 0.0, 2.0, 0.0, 5000.0, 0.0, 0.0, 0.0, 0.0]]
...
print("Total Length: "+str(sum(route_length)))
...
for k in range(1, len(gcode_list)):
    match gcode_list[k][0]:
        ...
        case _:
            print("Unknown G-Code: " + gcode_list[k][0])

# Insert Codes for Formulating the Circular Arc
# Write It as a Function
# Insert into the Plot

moving_point, = ax.plot([], [], [], marker='^', color='green', markersize=8)
arrow = ax.quiver(0, 0, 0, 0, 0, 0, color="red")  # Boş ok başlangıcı
    ...
    match gcode_list[k][0]:
        case "G1":
            ax.quiver(gcode_list[k - 1][1], gcode_list[k - 1][2], gcode_list[k - 1][3],
                      ...
                      gcode_list[k][3] - gcode_list[k - 1][3], length=5.0, normalize=True, color='black', alpha=0.25)
        case "G2":
            r = (gcode_list[k][6] ** 2 + gcode_list[k][7] ** 2) ** 0.5
            cx = gcode_list[k - 1][1] + gcode_list[k][6]
            cy = gcode_list[k - 1][2] + gcode_list[k][7]
        ...
        case "G5":
            ...
            ax.quiver(start[0], start[1], start[2], direction[0], direction[1], direction[2], length=5.0, normalize=True, color='black', alpha=0.25)
...
ani = FuncAnimation(fig, update, frames=partition_number, interval=100, blit=False)
