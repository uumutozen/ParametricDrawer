...
def interpolate_points(x, y, z, steps=100):
    # Find all interpolation points between two points
...
def cubic_bezier_derivative(t, control_points):
    #Calculates the derivative vector at a point on the Bezier curve.
    derivative_bernstein_coeffs = np.insert([comb(3, i, exact=True) * (3 - i) * (1 - t) ** (2 - i) * (-1) * t ** i for i in range(0, 3)],3,0)+np.insert([comb(3, i, exact=True) * i * (1 - t) ** (3 - i) * t ** (i - 1) for i in range(1, 4)],0,0)
...
def arc_length_integrand(t, control_points):
    #This function calculates the integrand part of an arc length integral of a Cubic Bezier curve.
...
def update(frame):
    ...
    # Calculate rotation (using e_interp_all only)
    e_angle = e_interp_all[frame]  # Angle in degrees
...
def convert_np_float64_list_to_ndarray(np_list):
    ...
    #Converts a list of type numpy float64 to ndarray.
...
# Initial corner coordinates of the tetrahedron
vertices = np.array([
    [0,0,0],    # Vertex 1
    [-2,2,8],  # Vertex 2
    [-2,-2,8],  # Vertex 3
...
for k in range(1, len(gcode_list)):
    ...
    match gcode_list[k][0]:
        ...
        case "G5":
            # Interpolate for Bezier curve
            t_values = np.linspace(0, 1, stepsize)
            bezier_points = np.array([cubic_bezier(t, cpts) for t in t_values])

            # Update G5 points for movement
...
# Animation function
anim = FuncAnimation(fig, update, frames=len(x_interp_all), interval=100, repeat=True)
plt.show()
